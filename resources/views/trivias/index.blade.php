...
<div class="space-y-6">
    ...
    <div class="relative overflow-hidden rounded-2xl glass-panel p-6 sm:p-8 border border-indigo-500/20 shadow-2xl">
        ...
        <div class="absolute -bottom-24 -left-24 w-72 h-72 bg-emerald-600/10 rounded-full blur-3xl pointer-events-none"></div>

        <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="space-y-2 max-w-2xl">
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-semibold tracking-wide">
                ...
                <p class="text-slate-300 text-xs sm:text-sm leading-relaxed">
                    Pesquise e consulte trívias de Kaslow, Ilya, Jale, Elsaland, Saphael, Abertura do Tempo, Dimensão Alternativa, Tormentum e Ilha Sprite. Cada página contabiliza <strong>2 pontos por página</strong> no arquivo do personagem!
                </p>
                <p class="text-slate-400 text-[11px] pt-1">
                    Conteúdo inicial compilado e organizado por <a href="{{ route('credits') }}" class="text-indigo-400 hover:underline font-semibold">Rafael G C Santos</a>.
                </p>
            </div>
            ...
            <div class="glass-card p-5 rounded-xl border border-slate-700/60 min-w-[260px] space-y-3">
                ...
                <div class="w-full bg-slate-900 rounded-full h-2.5 overflow-hidden p-0.5 border border-slate-800">
                    <div id="progress-bar" class="bg-gradient-to-r from-amber-500 via-emerald-500 to-indigo-500 h-full rounded-full transition-all duration-500" style="width: 0%"></div>
                </div>
                <div class="flex items-center justify-between text-xs pt-1">
                    <span class="text-slate-400">Pontos Arquivo:</span>
                    <span class="font-semibold text-white"><span id="user-points-display">0</span> / <span class="text-amber-400 font-mono">{{ $maxGfPoints }} pts</span></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="glass-panel p-4 sm:p-5 rounded-xl space-y-4 border border-slate-800">
        ...
        <form method="GET" action="{{ route('trivias.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
            ...
            <div class="sm:col-span-6 relative">
                ...
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar por trívia, NPC, item ou coordenada (ex: 416, 229)..."
                ...
                @if(request('q'))
                    <a href="{{ route('trivias.index', array_filter(request()->except('q'))) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-white text-xs">
                        ✕ Limpar
                    </a>
                @endif
            </div>
            ...
            <div class="sm:col-span-4">
                <select name="npc" onchange="this.form.submit()" class="w-full py-2.5 px-3 rounded-xl bg-slate-900/90 border border-slate-700/80 text-white text-xs sm:text-sm focus:outline-none focus:border-indigo-500 transition-all">
            ...
            <div class="sm:col-span-2 flex items-center gap-2">
                <button type="submit" class="w-full py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-md shadow-indigo-600/30 transition-all flex items-center justify-center gap-1.5">
                    Filtrar
                </button>
                @if(request()->hasAny(['q', 'city', 'zone', 'npc']))
                    <a href="{{ route('trivias.index') }}" title="Resetar todos os filtros" class="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs transition-colors">
                        🔄
                    </a>
                @endif
            </div>
                    ...
                    @foreach($zones as $zone)
                        ...
                        <a href="{{ route('trivias.index', array_merge(request()->all(), ['zone' => $zone->slug])) }}" 
                           class="px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all flex items-center gap-1.5 {{ $isCurrentZone ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' : 'bg-slate-900/50 text-slate-400 hover:text-white hover:bg-slate-800' }}">
                            <span>{{ $zone->name }}</span>
                            <span class="px-1.5 py-0.2 rounded text-[10px] {{ $isCurrentZone ? 'bg-indigo-700 text-white' : 'bg-slate-800 text-slate-400' }}">{{ $zoneTriviaCount }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    @if($trivias->isEmpty())
        <div class="glass-panel p-12 rounded-2xl text-center space-y-3 border border-slate-800">
            ...
            <p class="text-slate-400 text-xs max-w-md mx-auto">
                Não encontramos nenhuma trívia com os filtros selecionados. Tente buscar por outros termos ou selecionar outra cidade/mapa.
            </p>
            <a href="{{ route('trivias.index') }}" class="inline-block mt-2 px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-500 transition-all">
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @foreach($trivias as $trivia)
                <div id="trivia-card-{{ $trivia->id }}" class="glass-card p-5 rounded-2xl flex flex-col justify-between space-y-4 border border-slate-800/80 transition-all">
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <div class="flex items-center gap-1.5 flex-wrap mb-1">
                                    <span class="px-2.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[10px] font-semibold">
                                        🏰 {{ $trivia->zone->city }}
                                    </span>
                                    <span class="px-2.5 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-[10px] font-medium">
                                        📍 {{ $trivia->zone->name }}
                                    </span>
                                </div>
                                <h2 class="font-heading font-bold text-base sm:text-lg text-white group-hover:text-indigo-300 transition-colors">
                    ...
                    <div class="space-y-2.5 bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
                        <div class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                            <span>Passo a Passo ({{ $trivia->steps->count() }} {{ $trivia->steps->count() == 1 ? 'etapa' : 'etapas' }})</span>
                        ...
                        <div class="space-y-2">
                            @foreach($trivia->steps as $step)
                                <div class="flex items-start gap-2.5 text-xs text-slate-300">
                                    <span class="w-5 h-5 rounded-md bg-indigo-950 text-indigo-300 border border-indigo-500/30 flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5">
                                        {{ $step->step_number }}
                                    </span>
                                    <div class="flex-grow leading-relaxed">
                                        <span>{!! preg_replace('/<([^>]+)>/', '<span class="text-indigo-300 font-medium">&lt;$1&gt;</span>', e($step->instruction)) !!}</span>

                                        <div class="flex flex-wrap items-center gap-1.5 mt-1.5">
                                            @if($step->npc_name)
                                                <span class="px-2 py-0.5 rounded bg-slate-800 text-slate-300 text-[10px] border border-slate-700/60">
                                                    👤 {{ $step->npc_name }}
                                                </span>
                                            @endif
                                            ...
                                            @if($step->item_required)
                                                <span class="px-2 py-0.5 rounded bg-amber-950/60 text-amber-300 text-[10px] border border-amber-500/30">
                                                    📦 {{ $step->item_required }}
                                                </span>
                                            @endif
                                        </div>
                    ...
                    <div class="pt-2 flex items-center justify-between text-xs text-slate-400 border-t border-slate-800/60">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('trivias.show', $trivia->id) }}#comments" class="hover:text-indigo-300 transition-colors flex items-center gap-1">
                                💬 {{ $trivia->comments->count() }} comentários
                            </a>
                            @if($trivia->guides->count() > 0)
                                <a href="{{ route('trivias.show', $trivia->id) }}#guides" class="text-teal-400 hover:underline flex items-center gap-1 font-medium">
                                    📚 {{ $trivia->guides->count() }} guia
                                </a>
                            @endif
                        </div>

                        <a href="{{ route('trivias.show', $trivia->id) }}" class="text-indigo-400 hover:text-indigo-300 font-semibold flex items-center gap-1">
                            Ver Guia Completo →
                        </a>
                    </div>
                </div>
...
<script>
    ...
    async function toggleTriviaCompletion(triviaId) {
        ...
        showToast(isDone ? 'Trívia desmarcada!' : 'Trívia marcada como concluída! 🎉');
    }
